...
class Machine:
    def __init__(self, gpio_pin=17):
        ...
        self.last_state = "off"
        # GPIO simulation - replace with actual GPIO control
    ...
    def should_stay_off(self):
        return (time.time() - self.last_off_time) < 1.0

    def update_display(self, image):
        if self.is_on or (not self.is_on and self.last_state == "on" and self.should_stay_off()):
            cv2.putText(image, "PROCESS: ON", (10, 150),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        else:
            cv2.putText(image, "PROCESS: OFF", (10, 150),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        
    def _set_gpio(self, state):
        """Simulate GPIO control - replace with actual RPi.GPIO code"""
        self.gpio_state = state
        print(f"GPIO set to: {'HIGH' if state else 'LOW'}")
class HandDetector:
    def __init__(self, model_path, device="CPU", conf_thresh=0.5, nms_thresh=0.5, input_size=480, skipper=5,draw=True):
        self.classNames = ["arm", "palm"]
        self.conf_thresh = conf_thresh
    ...
    def resize_pad(self, image, size=(480, 480)):
    ...
    def postprocess(self, pred, orig_size=480):
    ...
    def find(self, image):
        ...
        images[0] = images[0][:, :480-self.shift]
...
class Dashboard:
    def __init__(self, app):
        ...
        # Set window size for 7-inch screen (800x480)
        self.root.geometry("800x480")
        self.root.configure(bg="#f0f0f0")
        
        # Remove minimum window size constraint for small screen
        # self.root.minsize(900, 600)  # Comment out or remove this line
        ...
        self.style.configure('Control.TButton', font=('Arial', 8, 'bold'),  # Reduced from 10 to 8
                            background='#3498db', foreground='white')
        self.style.configure('Toggle.TButton', font=('Arial', 8, 'bold'),  # Reduced from 10 to 8
                            background='#e74c3c', foreground='white')
        self.style.configure('Exit.TButton', font=('Arial', 8, 'bold'),  # Reduced from 10 to 8
                            background='#e74c3c', foreground='white')
        self.style.configure('Status.TLabel', background='#34495e', foreground='white', 
                            font=('Arial', 8))  # Reduced from 10 to 8
        self.style.configure('Fps.TLabel', background='#34495e', foreground='#2ecc71', 
                            font=('Arial', 8, 'bold'))  # Reduced from 10 to 8
        self.style.configure('StatusIndicator.Running.TLabel', background='#2ecc71', 
                            foreground='white', font=('Arial', 8, 'bold'))  # Reduced from 10 to 8
        self.style.configure('StatusIndicator.Stopped.TLabel', background='#e74c3c', 
                            foreground='white', font=('Arial', 8, 'bold'))  # Reduced from 10 to 8

        # Create main frames with reduced padding
        self.title_frame = ttk.Frame(self.root, style='Title.TLabel', height=30)
        self.title_frame.pack(fill=tk.X, pady=(0, 5))  # Reduced pady
        
        # Video frame with fixed height for small screen
        self.video_frame = ttk.Frame(self.root, height=300)
        self.video_frame.pack(fill=tk.X, padx=10, pady=5)  # Reduced padding
        self.video_frame.pack_propagate(False)  # Prevent frame from resizing to content
        
        self.control_frame = ttk.Frame(self.root, style='Control.TFrame', height=80)
        self.control_frame.pack(fill=tk.X, padx=10, pady=5)  # Reduced padding
        self.control_frame.pack_propagate(False)  # Prevent frame from resizing to content
        
        self.status_frame = ttk.Frame(self.root, style='Status.TFrame', height=30)
        self.status_frame.pack(fill=tk.X, side=tk.BOTTOM)
        self.status_frame.pack_propagate(False)  # Prevent frame from resizing to content

        # Title with smaller padding
        title_label = ttk.Label(self.title_frame, text="HAND SAFETY MONITORING SYSTEM", 
                               style='Title.TLabel')
        title_label.pack(pady=5)  # Reduced pady

        # Video display
        self.label = ttk.Label(self.video_frame)
        self.label.pack(expand=True)

        # Control buttons with smaller font and tighter layout
        control_title = ttk.Label(self.control_frame, text="System Controls", 
                                 font=('Arial', 10, 'bold'), background='#ecf0f1')  # Reduced font
        control_title.grid(row=0, column=0, columnspan=4, pady=(0, 5))  # Reduced pady

        self.start_btn = ttk.Button(self.control_frame, text="Start",  # Shorter text
                                   command=self.start, style='Control.TButton')
        self.start_btn.grid(row=1, column=0, padx=2, pady=2, sticky="ew")  # Reduced padding

        self.stop_btn = ttk.Button(self.control_frame, text="Stop",  # Shorter text
                                  command=self.stop, style='Control.TButton')
        self.stop_btn.grid(row=1, column=1, padx=2, pady=2, sticky="ew")  # Reduced padding

        self.toggle_btn = ttk.Button(self.control_frame, text="Video: OFF",  # Shorter text
                                    command=self.toggle_feedback, style='Toggle.TButton')
        self.toggle_btn.grid(row=1, column=2, padx=2, pady=2, sticky="ew")  # Reduced padding

        self.exit_btn = ttk.Button(self.control_frame, text="Exit",  # Shorter text
                                  command=self.exit_app, style='Exit.TButton')
        self.exit_btn.grid(row=1, column=3, padx=2, pady=2, sticky="ew")  # Reduced padding

        # Configure grid weights for responsive layout
        for i in range(4):
            self.control_frame.columnconfigure(i, weight=1)

        # Status bar with smaller elements
        self.status_indicator = ttk.Label(self.status_frame, text="STOPPED", 
                                        style='StatusIndicator.Stopped.TLabel')
        self.status_indicator.pack(side=tk.LEFT, padx=(5, 2), pady=2)  # Reduced padding

        self.status_label = ttk.Label(self.status_frame, text="Status: Stopped",  # Shorter text
                                     style='Status.TLabel')
        self.status_label.pack(side=tk.LEFT, padx=(0, 5), pady=2)  # Reduced padding

        self.fps_label = ttk.Label(self.status_frame, text="FPS: 0", style='Fps.TLabel')
        self.fps_label.pack(side=tk.RIGHT, padx=5, pady=2)  # Reduced padding

        # System info with smaller text
        self.info_label = ttk.Label(self.status_frame, text="v1.0",  # Shorter text
                                   style='Status.TLabel')
        self.info_label.pack(side=tk.RIGHT, padx=5, pady=2)  # Reduced padding

        # Initialize
        self.thread = None
        
        # Load your custom placeholder image - resize for small screen
        try:
            # Load and resize your image to fit the smaller display area
            self.placeholder_img = Image.open("intern\hand_detector\placeholder.jpg")
            self.placeholder_img = self.placeholder_img.resize((600, 280), Image.LANCZOS)  # Smaller size
        except Exception as e:
            print(f"Error loading placeholder image: {e}")
            # Fallback to a simple text image if your image can't be loaded
            self.placeholder_img = Image.new('RGB', (600, 280), (240, 240, 240))
            ...
            position = ((600 - text_width) // 2, (280 - text_height) // 2)
        ...
        # Make the window fullscreen (optional)
        # self.root.attributes('-fullscreen', True)
        
        # Add keyboard shortcut for exiting fullscreen (if enabled)
    ...
    def start(self):
        if not self.app.running:
            ...
            self.status_label.config(text="Status: Active")  # Shorter text
    ...
    def stop(self):
        ...
        self.status_label.config(text="Status: Stopped")  # Shorter text
    ...
    def toggle_feedback(self):
        ...
        self.toggle_btn.config(text=f"Video: {state}")  # Shorter text
        
        # Show placeholder image immediately when feedback is disabled
    ...
    def update_image(self):
        if self.app.show_feedback:
            try:
                ...
                # Resize image to fit small screen
                img = img.resize((600, 280), Image.LANCZOS)
                self.current_img = img
            except queue.Empty:
                pass  # Keep the current image if queue is empty
        else:
            # Use your custom placeholder image
            self.current_img = self.placeholder_img
