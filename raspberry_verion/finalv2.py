...
drawbox = True
device = 0
confidence_threshol = 0.8
motion_sensibility = 25
motion_scale = 0.50
frame_skipper = 14
yoloPath = "yolo-Weights/lastbest_yolov8_640_openvino_model/lastbest.xml"



class Application:
    def __init__(self, video_path, model_path, frame_skipper, Fps):
        self.cap = cv2.VideoCapture(0)  # Use default camera for demomp        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
        ...
        self.detector = ONNXHandDetector(
            model_path="yolo-Weights/inference_model_rf_detr_batch2.onnx",
            ...
            try:
                ...
                if not motion_detected:
                    if self.j == int((self.skipper / 2)):
                        frame = self.resize_to_fixed_height(frame, 640)
                ...
                if motion_detected or hand_detected:
                    if self.machine.is_on:
                      self.machine.turn_off()
                       ...
                       if not self.machine.is_on and not self.machine.should_stay_off():
                          self.machine.turn_on()
                
                # Relay logic: only switch when state needs to change
